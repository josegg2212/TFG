...
class CCommandExtractor(Transformer):
    ...
    def object_name(self, items):
        self.entities["object"] = str(items[0])

    def object_complement(self, items):
        self.entities["object"] = " ".join([self.entities["object"], ''.join(items)])

    def point(self, items):
        self.entities["point"] = str(items[0])

    def table(self, items):
        self.entities["table"] = str(items[0])
